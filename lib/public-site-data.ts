...
export async function getPublicServices() {
  const organization = await resolvePublicConstructOrganization();
  if (!organization) return legacyPrisma.service.findMany({ where: { isActive: true }, orderBy: { displayOrder: "asc" } });
  const items = await getConstructPrisma().service.findMany({ where: { organizationId: organization.id, isActive: true }, orderBy: { displayOrder: "asc" } }); return items.map(mapService);
}
export async function getPublicServiceBySlug(slug: string) { const organization = await resolvePublicConstructOrganization(); if (!organization) return legacyPrisma.service.findFirst({ where: { slug } }); const item = await getConstructPrisma().service.findFirst({ where: { organizationId: organization.id, slug } }); return item ? mapService(item) : null; }
export async function getRelatedPublicServices(id: string | number, take = 3) { const organization = await resolvePublicConstructOrganization(); if (!organization) return legacyPrisma.service.findMany({ where: { isActive: true, id: { not: Number(id) } }, orderBy: { displayOrder: "asc" }, take }); const items = await getConstructPrisma().service.findMany({ where: { organizationId: organization.id, isActive: true, id: { not: String(id) } }, orderBy: { displayOrder: "asc" }, take }); return items.map(mapService); }

export async function getPublicProjects(filters?: { status?: string; featured?: boolean; category?: string; take?: number }) {
  const organization = await resolvePublicConstructOrganization();
...
export async function getPublicProjectBySlug(slug: string) { const projects = await getPublicProjects(); return projects.find(project => project.slug === slug) ?? null; }
export async function getRelatedPublicProjects(project: PublicProject, take = 3) { const projects = await getPublicProjects({ category: project.category }); return projects.filter(item => item.id !== project.id).slice(0, take); }
...
export async function getPublicClients(filters?: { featured?: boolean; take?: number }): Promise<PublicClient[]> { const organization = await resolvePublicConstructOrganization(); if (!organization) return legacyPrisma.client.findMany({ where: { active: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: { displayOrder: "asc" }, take: filters?.take }); const items = await getConstructPrisma().client.findMany({ where: { organizationId: organization.id, isActive: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: [{ displayOrder: "asc" }, { name: "asc" }], take: filters?.take }); return items.map(item => ({ ...item, logo: item.logoUrl, active: item.isActive })); }
export async function getPublicTestimonials(filters?: { featured?: boolean; take?: number }): Promise<PublicTestimonial[]> { const organization = await resolvePublicConstructOrganization(); if (!organization) return legacyPrisma.testimonial.findMany({ where: { active: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: { displayOrder: "asc" }, take: filters?.take }); const items = await getConstructPrisma().testimonial.findMany({ where: { organizationId: organization.id, isActive: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: [{ displayOrder: "asc" }, { clientName: "asc" }], take: filters?.take }); return items.map(item => ({ ...item, photo: item.photoUrl, active: item.isActive })); }
export async function getPublicFaqs(filters?: { featured?: boolean; take?: number }): Promise<PublicFaq[]> { const organization = await resolvePublicConstructOrganization(); if (!organization) return legacyPrisma.fAQ.findMany({ where: { active: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: { displayOrder: "asc" }, take: filters?.take }); const items = await getConstructPrisma().faq.findMany({ where: { organizationId: organization.id, isActive: true, ...(filters?.featured !== undefined ? { featured: filters.featured } : {}) }, orderBy: [{ displayOrder: "asc" }, { question: "asc" }], take: filters?.take }); return items.map(item => ({ ...item, active: item.isActive })); }
export async function getPublicStats() { const organization = await resolvePublicConstructOrganization(); if (!organization) return Promise.all([legacyPrisma.project.count({ where: { status: "Completed" } }), legacyPrisma.client.count({ where: { active: true } }), legacyPrisma.service.count({ where: { isActive: true } }), legacyPrisma.teamMember.count({ where: { isActive: true } })]); const db = getConstructPrisma(); return Promise.all([db.project.count({ where: { organizationId: organization.id, status: "Completed" } }), db.client.count({ where: { organizationId: organization.id, isActive: true } }), db.service.count({ where: { organizationId: organization.id, isActive: true } }), db.teamMember.count({ where: { organizationId: organization.id, isActive: true } })]); }
