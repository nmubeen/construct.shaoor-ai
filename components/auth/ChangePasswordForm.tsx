...
export default function ChangePasswordForm() {
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <button disabled={pending} className="w-full rounded-lg bg-(image:--gradient-button-bg) py-3 font-medium text-white disabled:opacity-50">{pending ? "Updating…" : "Update Password"}</button>
    </form>
  );
}
