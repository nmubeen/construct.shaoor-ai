...
function isAllowedFile(file: File) {
  ...
  return ALLOWED_MEDIA_EXTENSIONS.includes(
    ext as (typeof ALLOWED_MEDIA_EXTENSIONS)[number],
  );
}

export default function MediaUpload({ folders }: { folders: string[] }) {
    ...
    startTransition(async () => {
      ...
      for (const item of next) {
        ...
        try {
          ...
          item.status = "error";
          item.message =
            error instanceof Error ? error.message : Messages.uploadFailed;
        }

        setFiles([...next]);
      }

      const uploadedCount = next.filter(
      ...
      <div className="space-y-6 rounded-xl bg-white p-6 shadow sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm text-slate-700">
            <FaFolderTree />
            <span>
              Uploading to: <strong>{folder || "uploads root"}</strong>
            </span>
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-xs text-slate-600">
            ...
            <p className="mt-1">
              Max file size: {formatFileSize(MAX_MEDIA_FILE_SIZE)}
            </p>
          </div>
        </div>
