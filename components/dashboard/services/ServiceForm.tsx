...
export function ServiceForm({ service, subServices, images, folders }: { service?: Service; subServices?: string[]; images: PickableImage[]; folders?: PickerFolder[] }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <section className="grid gap-4 rounded-lg border border-slate-200 bg-(image:--gradient-form-bg) p-6 shadow-sm md:grid-cols-2">
      ...
      <ImageUrlField label="Image URL" name="imageUrl" defaultValue={service?.imageUrl} hint="A direct link to an image representing this service. Must be a valid URL. Landscape, roughly 3:2 (e.g. 1200×800px) — shown cropped to fill both a card thumbnail and a taller detail-page image. Optional." images={images} folders={folders} />
    ...
    <section className="rounded-lg border border-slate-200 bg-(image:--gradient-form-bg) p-6 shadow-sm"><h2 className="font-bold text-(--color-primary-text)">Sub-services</h2><p className="text-xs text-slate-500">One per line, up to 10. Shown as bullet points under this service on the public website.</p><textarea className={`${input} min-h-36`} name="subServices" defaultValue={subServices?.join("\n") ?? ""} /></section>
    <section className="grid gap-4 rounded-lg border border-slate-200 bg-(image:--gradient-form-bg) p-6 shadow-sm md:grid-cols-2"><h2 className="text-lg font-bold text-(--color-primary-text) md:col-span-2">SEO</h2>
    ...
    <div className="sticky bottom-4 flex justify-end rounded-lg border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur"><button className="rounded-md bg-(image:--gradient-button-bg) px-6 py-3 text-sm font-semibold text-white hover:brightness-110">{service ? "Save changes" : "Create service"}</button></div>
  </form>;
}
