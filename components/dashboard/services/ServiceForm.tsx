...
export function ServiceForm({ service, error }: { service?: Service; error?: string }) {
  return <form action={saveConstructServiceAction} className="space-y-5">
    ...
    <div className="flex justify-end"><button className="rounded-md bg-[#094136] px-6 py-3 text-sm font-semibold text-white hover:bg-[#7D9D76]">{service ? "Save changes" : "Create service"}</button></div>
  </form>;
}
