...
export function ConstructDashboardSidebar({ organizationName, role, organizationStatus }: { organizationName: string; role: string; organizationStatus: string }) {
  return (
    <aside className="flex w-full flex-col border-b border-slate-800 bg-slate-950 text-white lg:fixed lg:inset-y-0 lg:w-72 lg:border-r">
      <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
        <span className="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-[#0E4A7B] to-teal-500 text-lg font-black">S</span>
        <div className="min-w-0"><p className="font-bold">Shaoor Construct</p><p className="truncate text-xs text-slate-400">{organizationName}</p></div>
      </div>
      <nav className="grid grid-cols-2 gap-1 p-3 sm:grid-cols-4 lg:flex lg:flex-1 lg:flex-col">
        {navigation.map(({ href, label, icon: Icon, available }) => available ? (
          <Link key={href} href={href} className="flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2.5 text-sm font-medium"><Icon className="size-4" />{label}</Link>
        ) : (
      ...
      <div className="border-t border-white/10 p-4">
        <div className="mb-3 flex items-center gap-3 rounded-xl bg-white/5 p-3"><Building2 className="size-4 text-teal-400" /><div><p className="text-xs font-semibold">{role}</p><p className="text-xs text-slate-500">{organizationStatus.toLowerCase()} workspace</p></div></div>
