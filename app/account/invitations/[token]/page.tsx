...
export default async function InvitationPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ error?: string; sent?: string }> }) {
  ...
  return <ConstructAuthShell eyebrow="Workspace invitation" title={usable ? `Join ${invitation.organization.name}` : "Invitation unavailable"} description={usable ? `You have been invited as ${invitation.role.toLowerCase()}.` : "This invitation is invalid, expired, revoked, or already accepted."}>
    {query.error && <p className="mb-4 rounded-md bg-red-50 p-3 text-sm text-red-700">{query.error}</p>}{query.sent && <p className="mb-4 rounded-md bg-[#eef3ec] p-3 text-sm text-[#7D9D76]">A secure sign-in link was sent to {invitation?.email}.</p>}
    {usable && (matches ? <form action={acceptConstructInvitationAction}><input type="hidden" name="token" value={token}/><button className="w-full rounded-md bg-[#094136] px-4 py-3 font-semibold text-white">Accept invitation</button></form> : context?.authUser ? <div className="space-y-3"><p className="rounded-md bg-amber-50 p-3 text-sm text-amber-800">You are signed in as {signedInEmail}. This invitation belongs to {invitation.email}.</p><Link href={`/account/login?next=${encodeURIComponent(`/account/invitations/${token}`)}`} className="block text-center text-sm font-semibold text-[#7D9D76]">Sign in with the invited account</Link></div> : <div className="space-y-3"><form action={sendConstructInvitationSignInLinkAction}><input type="hidden" name="token" value={token}/><button className="w-full rounded-md bg-[#094136] px-4 py-3 font-semibold text-white">Email me a secure sign-in link</button></form><p className="text-center text-xs text-slate-500">The link will be sent only to {invitation.email}.</p><Link href={`/account/login?next=${encodeURIComponent(`/account/invitations/${token}`)}`} className="block text-center text-sm font-semibold text-[#7D9D76]">I already have a password</Link></div>)}
  </ConstructAuthShell>;
}
