  ...
  return (
    <ConstructAuthShell eyebrow="Account recovery" title="Choose a new password" description="Use at least 8 characters with uppercase, lowercase, a number and a special character.">
      <form action={updateConstructPasswordAction} className="space-y-5">
        ...
        <label className="block text-sm font-semibold text-slate-700">New password<ConstructPasswordInput name="password" autoComplete="new-password" minLength={8} required className="mt-2 w-full rounded-md border border-[#7D9D76] px-4 py-3 outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-100" /></label>
        <label className="block text-sm font-semibold text-slate-700">Confirm password<ConstructPasswordInput name="confirmation" autoComplete="new-password" minLength={8} required className="mt-2 w-full rounded-md border border-[#7D9D76] px-4 py-3 outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-100" /></label>
