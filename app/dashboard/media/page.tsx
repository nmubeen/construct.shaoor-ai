  ...
  return (
    <details open style={{ marginLeft: node.depth > 0 ? "0.9rem" : 0 }}>
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 rounded-md py-1 pr-1 text-sm marker:content-none">
        <a href={`/dashboard/media?folder=${node.id}`} className={`flex min-w-0 flex-1 items-center gap-1.5 truncate rounded-md px-1.5 py-1 font-medium ${isActive ? "bg-[#eef3ec] text-(--color-primary-text)" : "text-slate-700 hover:bg-slate-100"}`}>
...
export default async function MediaPage({ searchParams }: { searchParams: Promise<{ deleted?: string; error?: string; folderCreated?: string; folderDeleted?: string; folder?: string }> }) {
  ...
  return <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    <header className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.2em] text-(--color-secondary-text-icon)">Website CMS</p><h1 className="mt-2 text-3xl font-bold text-(--color-primary-text)">Media Library</h1><p className="mt-2 text-sm text-slate-600">Upload reusable images and documents for this tenant&apos;s website.</p></header>
    {query.deleted && <p className="mb-5 rounded-md border border-[#7D9D76]/40 bg-[#eef3ec] p-3 text-sm text-(--color-secondary-text-icon)">Media deleted successfully.</p>}
    {query.folderCreated && <p className="mb-5 rounded-md border border-[#7D9D76]/40 bg-[#eef3ec] p-3 text-sm text-(--color-secondary-text-icon)">Folder created successfully.</p>}
    {query.folderDeleted && <p className="mb-5 rounded-md border border-[#7D9D76]/40 bg-[#eef3ec] p-3 text-sm text-(--color-secondary-text-icon)">Folder deleted successfully.</p>}
    ...
    <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
      <aside className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="mb-3 flex items-center gap-2 text-sm font-bold text-slate-950"><Folder className="size-4 text-(--color-secondary-text-icon)" />Folders</h2>
        <a href="/dashboard/media" className={`mb-1 flex items-center gap-1.5 rounded-md px-1.5 py-1 text-sm font-medium ${!activeFolder ? "bg-[#eef3ec] text-(--color-primary-text)" : "text-slate-700 hover:bg-slate-100"}`}>All media</a>
        <a href="/dashboard/media?folder=none" className={`mb-2 flex items-center gap-1.5 rounded-md px-1.5 py-1 text-sm font-medium ${activeFolder === "none" ? "bg-[#eef3ec] text-(--color-primary-text)" : "text-slate-700 hover:bg-slate-100"}`}>No folder <span className="text-xs font-normal text-slate-400">({unfiledCount[0]?.count ?? 0})</span></a>
        ...
        {canEdit && <form action={createConstructMediaFolderAction} className="mt-4 space-y-2 border-t border-slate-100 pt-4">
          ...
          <button className="w-full rounded-md bg-(image:--gradient-button-bg) px-3 py-2 text-xs font-semibold text-white">Create folder</button>
        </form>}
      </aside>
      <div>
        {activeFolderName && <p className="mb-3 text-sm text-slate-600">Showing: <strong>{activeFolderName}</strong> · <a href="/dashboard/media" className="text-(--color-secondary-text-icon) underline">clear filter</a></p>}
